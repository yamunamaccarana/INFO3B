...
import unibg.saoms.msg.EEMessage.Event;
...
public class Environment extends Task implements TaskInterface {
	...
	public void execute() {
		...
		case 0:
			...
			break;
		case 1:
			// Test1: new airplane landing
			EEMessage t1 = new EEMessage(Event.LANDING, 4);
			System.out.println("Enviroment: Test1: "+t1.toString()+" at "+new Timestamp(date.getTime())+" ");
			try {
				send(ETopic, t1);
		...
		case 2:
			...
			System.out.println("Enviroment: Test2: "+t2.toString()+" at "+new Timestamp(date.getTime())+" ");
			try {
				send(ETopic, t2);
			} catch (TopicNotFoundException e) {e.printStackTrace();}
			it++;
		...
		case 3:
			...
			System.out.println("Enviroment: Test3: "+t3.toString()+" at "+new Timestamp(date.getTime())+" ");
			try {
				send(ETopic, t3);
			} catch (TopicNotFoundException e) {e.printStackTrace();}
			it++;
		...
		case 4:
			...
			System.out.println("Enviroment: Test4: "+t4.toString()+" at "+new Timestamp(date.getTime())+" ");
			try {
				send(ETopic, t4);
			} catch (TopicNotFoundException e) {e.printStackTrace();}
			it++;
		...
		case 5:
			...
			System.out.println("Enviroment: Test5: "+t5.toString()+" at "+new Timestamp(date.getTime())+" ");
			try {
				send(ETopic, t5);
			} catch (TopicNotFoundException e) {e.printStackTrace();}
			it++;
